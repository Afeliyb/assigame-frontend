...
export function ContactButtons({
  ...
  const handleMessage = () => {
    ...
    // Redirige vers le chat avec le vendeur, en précisant le produit si disponible
  ...
  const handleRevealContact = async () => {
    ...
    if (contactState.status === "revealed") return; // déjà affiché
        ...
        {contactState.status === "revealed" && (
          <div className="flex flex-col gap-1.5 p-3.5 rounded-2xl bg-[var(--surface-elevated)] border border-[var(--border-subtle)]">
            ...
            {whatsappDigits && (
              ...
              >
                {/* WhatsApp icon (SVG inline pour ne pas dépendre d'une lib) */}
