...
import { MessageSquare, Phone, ChevronDown, Loader2, Copy, Check } from "lucide-react";
...
type Props = {
  ...
  productTitle?: string;
  productId?: string;
  productPrice?: number; // ── AJOUT OPTIONNEL : Pour passer le prix au chat
  productImage?: string; // ── AJOUT OPTIONNEL : Pour passer la miniature au chat
  disabled?: boolean;
};
...
export function ContactButtons({
  ...
  const [loginModal, setLoginModal]   = useState<"idle" | "message" | "contact">("idle");
  ...
  const handleMessage = () => {
    ...
    // On construit proprement les paramètres d'URL pour le chat
    const params = new URLSearchParams();
    if (productId) params.set("produit", productId);
    if (productTitle) params.set("titre", productTitle);
    if (productPrice) params.set("prix", productPrice.toString());
    if (productImage) params.set("image", productImage);

    const queryString = params.toString();
    const href = queryString
      ? `/dashboard/messages/${sellerId}?${queryString}`
  ...
  const handleRevealContact = async () => {
    ...
    try {
      ...
      setContactState({ status: "revealed", telephone: info.telephone, whatsapp: info.whatsapp });
    } catch (e) {
      setContactState({
        status: "error",
        message: e instanceof ApiError ? e.message : "Impossible de récupérer les coordonnées.",
      });
    }
  };
  ...
  const contactNumber =
    contactState.status === "revealed"
      ...
      : null;
  const whatsappDigits = contactNumber ? sanitizePhoneDigits(contactNumber) : null;
            ...
            {contactState.telephone && (
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="text-sm font-semibold">{contactState.telephone}</span>
                </a>
                <button
