...
export default function HomePage() {
  ...
  const onlineListings = useMemo(
    () => listings.filter((p) => p.status === "En ligne"),
    [listings],
  );
  ...
  const trustedSellers = useMemo(() => {
    const map = new Map<
      string,
      NonNullable<(typeof onlineListings)[number]["seller"]>
    >();
        ...
        {!introFinished && (
          ...
          >
            <div className="text-center font-display font-black text-5xl md:text-7xl tracking-tight leading-none overflow-hidden">
              ...
              >
                <div className="text-base font-sans font-medium text-black/40 dark:text-white/40 mt-4 tracking-normal flex items-center justify-center gap-2">
                  <TogoFlag className="w-5 h-5" /> Le marketplace togolais
                </div>
              </motion.div>
            </div>
          </motion.div>
      ...
      <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-10">
        ...
        <Reveal className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center py-12 lg:py-16 mb-4">
          <div className="flex flex-col items-start gap-5">
            <div className="flex items-center gap-2 px-3.5 py-1.5 bg-[var(--surface-elevated)] rounded-full">
              ...
              <span className="text-xs font-bold flex items-center gap-1.5">
                <TogoFlag /> Marketplace togolais N°1
              </span>
            </div>
            ...
            {stats.total > 0 && (
              <div className="flex items-center gap-5 text-sm">
                <div className="flex flex-col">
                  <span className="font-black text-2xl leading-tight">
                    {stats.total}
                  </span>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <Link
                href="/browse"
                className="inline-flex items-center gap-2 px-6 py-3.5 bg-[var(--foreground)] text-[var(--background)] rounded-full font-bold text-sm hover:scale-105 transition-transform"
              >
                {t.home_cta_explore} <ArrowRight className="w-4 h-4" />
              </Link>
              <Link
                href="/dashboard/new"
                className="inline-flex items-center gap-2 px-6 py-3.5 border border-[var(--border-subtle)] rounded-full font-bold text-sm hover:bg-[var(--surface-elevated)] transition-colors"
              >
                {t.home_cta_sell}
              </Link>
            </div>
          ...
          {isLoading ? (
            ...
            <Link
              href={`/product/${heroShowcase.id}`}
              className="hidden lg:block h-72"
        ...
        {!isLoading && !error && featured && (
          <div className="grid grid-cols-1 lg:grid-cols-6 gap-4 mb-16 auto-rows-[200px]">
            ...
            >
              <motion.div
                className="w-full h-full min-h-[280px] rounded-[1.75rem] overflow-hidden relative group"
                whileHover={{ scale: 0.99 }}
                transition={{ type: "spring", stiffness: 400, damping: 30 }}
              >
                <Image
                ...
                <div className="absolute bottom-5 left-5 right-5 text-white">
                  <h3 className="font-display font-bold text-xl lg:text-2xl line-clamp-2 mb-2">
                    {featured.title}
                  </h3>
            ...
            {secondary && (
              <Link
                href={`/product/${secondary.id}`}
                className="lg:col-span-3 lg:row-span-1"
                onMouseEnter={() => setCursorLabel("Voir")}
                onMouseLeave={() => setCursorLabel(null)}
              >
                <motion.div
                  className="w-full h-full min-h-[150px] rounded-[1.75rem] overflow-hidden relative group"
                  whileHover={{ scale: 0.99 }}
                  transition={{ type: "spring", stiffness: 400, damping: 30 }}
                >
                  <Image
                  ...
                  <div className="absolute bottom-5 left-5 right-5 text-white">
                    ...
                    <span className="font-black text-base">
                      {secondary.price.toLocaleString("fr-FR")} FCFA
                    </span>
                  </div>
                </motion.div>
              </Link>
              ...
              <div>
                ...
                <div className="flex -space-x-3">
                  {trustedSellers.slice(0, 4).map((s) => (
                    <div
                      key={s.id}
                      className="w-10 h-10 rounded-full border-2 border-[var(--background)] overflow-hidden"
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img
                        src={s.avatar || getAvatarUrl(s.name)}
                        alt={s.name}
        ...
        {categories.length > 0 && (
          <Reveal className="mb-16">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-2">
              {categories.map((cat) => {
                const Icon = cat.icon
                  ? (ICON_MAP[cat.icon] ?? Package)
                  : Package;
                const count = onlineListings.filter(
          ...
          <Reveal className="mb-16 -mx-4 sm:-mx-6 lg:-mx-10">
            <InfiniteMarquee speed={30} className="py-3">
              {categories.concat(categories).map((cat, i) => (
                <div
                  key={`${cat.id}-${i}`}
                  className="mx-3 px-5 py-2.5 rounded-full border border-[var(--border-subtle)] bg-[var(--surface-elevated)] text-sm font-bold flex items-center gap-2 whitespace-nowrap"
                >
                  <TogoFlag /> {cat.name}{" "}
          ...
          <Reveal className="mb-16">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-black/50 dark:text-white/50 mt-1 flex items-center gap-1.5">
        ...
        {trending.length > 0 && (
          <Reveal className="mb-16">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link
                href="/browse"
                className="text-xs font-bold uppercase tracking-wider opacity-50 hover:opacity-100 transition-opacity flex items-center gap-1"
              >
